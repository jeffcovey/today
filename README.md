...
- **Unison never silently destroys a version.** When both sides changed a file, the winner is chosen by `prefer` under `[deployments.<provider>.<name>.unison]` — `"newer"` (default, by mtime), `"older"`, `"local"` (this device) or `"remote"` (the sync target) — and the loser is kept beside the original as `name (conflict_on_YYYY-MM-DD).md`. Independently, the previous version of every `*.md` Unison overwrites (conflict or not) is kept on the replica that was overwritten, under its Unison directory (`~/.unison/backup` on a droplet, `.data/unison/backup` in the sync container); tune with `max_backups = 5` or turn off with `backups = false`. If another sync tool (Obsidian Sync, iCloud) also touches the same vault, set `prefer = "remote"` on the device it runs on so a stale copy it replays can never overwrite the server.
