...
async function main() {
  ...
  // Group simple tasks by status for better organization
  const statusGroups = {};
  ...
  // Read existing tasks.md content to append to
  let existingContent = '';
  if (fs.existsSync(outputFile)) {
    existingContent = fs.readFileSync(outputFile, 'utf8');
  }

  // Build new content for tasks.md
  let newTasksContent = '';

  if (!existingContent) {
    // Add header if file is new
    newTasksContent = '# Tasks\n\n';
    newTasksContent += 'Tasks migrated from Notion Action Items database.\n\n';
  }

  // Add a separator for this batch
  const now = new Date().toISOString();
  newTasksContent += `\n## Migrated Tasks - ${now}\n\n`;

  // Add tasks grouped by status
  for (const [status, tasks] of Object.entries(statusGroups)) {
    newTasksContent += `### ${status}\n\n`;

    for (const task of tasks) {
      newTasksContent += formatAsObsidianTask(task) + '\n';
  ...
  // Append to tasks.md
  fs.appendFileSync(outputFile, newTasksContent);
